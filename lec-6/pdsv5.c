...
int pds_load_ndx()
{
    ...
    for (int i = 0; i < repo_handle.rec_count; i++)
    {
        ...
        // adding the node
        if (bst_add_node(&repo_handle.ndx_root, temp->key, temp) != BST_SUCCESS)
...
int get_rec_by_key(int key, void *rec)
{
    ...
    {
        if (fseek(repo_handle.pds_data_fp, ((struct PDS_NdxInfo *)temp->data)->offset, SEEK_SET))
            return PDS_FILE_ERROR;
    ...
    *comp_count = 0;
    ...
    while (fread(&key, sizeof(int), 1, repo_handle.pds_data_fp))
    {
        ...
        if (matcher(rec, searchvalue) == 0)
        {
            ...
            if (((struct PDS_NdxInfo *)bst_search(repo_handle.ndx_root, key)->data)->is_deleted != REC_NOT_DELETED)
                return PDS_REC_NOT_FOUND;
...
int delete_rec_by_key(int key)
{
    ...
    struct PDS_NdxInfo *ndx_to_delete = bst_search(repo_handle.ndx_root, key)->data;
...
int bst_pre_order_traversal(struct BST_Node *root)
{
    ...
    {
        // writing to file
        if (fwrite(root->data, sizeof(struct PDS_NdxInfo), 1, repo_handle.pds_ndx_fp) < 1)
            return PDS_FILE_ERROR;
